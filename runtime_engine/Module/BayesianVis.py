...
def plot_uncertainty_decomposition(x: np.ndarray, y: np.ndarray,
    ...
    When heteroscedastic=False, the aleatoric panel is replaced with
    a note that aleatoric uncertainty is not modeled.
    """
    fig = plt.figure(figsize=figsize)
    ...
    if heteroscedastic:
        ...
        ax7.text(0.5, 0.5, 'Aleatoric uncertainty\nnot modeled\n(heteroscedastic=False)',
                 ha='center', va='center', transform=ax7.transAxes,
                 fontsize=10, color='gray',
                 bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
        ax7.set_title('Aleatoric (N/A)')
